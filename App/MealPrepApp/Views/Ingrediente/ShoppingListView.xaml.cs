...
{
    public ShoppingListView() => InitializeComponent();

    private void OnPrintClick(object sender, RoutedEventArgs e)
    {
        // Button is IsEnabled-bound to Rows.Count, so the empty case can't happen here.
        if (DataContext is not ShoppingListViewModel vm || vm.Rows.Count == 0)
            return;
        ...
        print.PrintDocument(((IDocumentPaginatorSource)doc).DocumentPaginator,
            "Lista de cumparaturi");
    }

    private static FlowDocument BuildPrintDocument(ShoppingListViewModel vm)
    {
